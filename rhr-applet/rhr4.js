...
var angles = [0, Math.PI/4, Math.PI/2, 3*Math.PI/4, Math.PI,
            5*Math.PI/4, 3*Math.PI/2, 7*Math.PI/4, "+z", "-z"];
...
function check(guess) {
    ...
    if (guess == r.crossDir(p)) {
        ...
        ctx2.fillText("Incorrect! Correct answer: " + r.crossDir(p) + ".", feedback.width/3.5, feedback.height/2);
    }
}